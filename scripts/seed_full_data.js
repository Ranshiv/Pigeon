...
// Same default URI as inspect_db.js
...
async function seedData() {
    ...
    try {
        ...
        // 1. Seed or Upgrade APIs
        console.log('Upserting APIs...');
        let apiCount = 0;
        for (const api of publicApiCatalog) {
            await db.collection('marketplaceapis').updateOne(
                { id: api.id },
                { $set: api },
                { upsert: true }
            );
        ...
        console.log(`Ensured ${apiCount} APIs exist.`);
        ...
        for (const api of publicApiCatalog) {
            ...
            await db.collection('guides').updateOne(
                ...
                {
                    $set: guideData,
                    $setOnInsert: { createdAt: new Date() } // Only set creation date on insert
                },
                { upsert: true }
            );
            guideCount++;
        }
        console.log(`Ensured ${guideCount} Guides exist.`);
