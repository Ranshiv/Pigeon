    ...
    return <main className="tg-root">
        ...
        <section className="tg-layout">
            ...
            <section className="tg-review">
                ...
                {!activeSuite && <div className="tg-suites"><h3>Recent suites</h3>{suites.map((suite) => <button type="button" key={suite._id} onClick={() => openSuite(suite._id)}><FiFileText /><span><b>{suite.name}</b><small>{suite.caseCount} cases{suite.updatedAt ? ` · ${new Date(suite.updatedAt).toLocaleDateString()}` : ''}</small></span><em className={`tg-suite-status tg-suite-status--${suite.status}`}>{suite.status}</em></button>)}{!suites.length && <div className="tg-empty-state"><FiShield /><h3>No generated suites yet</h3><p>Select source evidence on the left to create the first reviewable draft.</p></div>}</div>}
                {activeSuite && <>
