...
function ProductPreview({ view }) {
    ...
    if (view.id === 'network') return <div className="pigeon-product-preview pigeon-product-preview--network" aria-label="API Network preview"><div className="pigeon-preview-topbar"><span><Icon /> {view.badge}</span><b><i className="pigeon-live-dot" /> 12 services mapped</b></div><div className="pigeon-network-map" aria-hidden="true"><svg viewBox="0 0 520 220" preserveAspectRatio="none"><path d="M260 106 L83 42 M260 106 L437 42 M260 106 L451 154 M260 106 L79 154" /><path className="is-active" d="M260 106 L437 42" /></svg><span className="node node--main"><i />API Gateway<small>9 routes</small></span><span className="node node--one"><i />GraphQL<small>schema</small></span><span className="node node--two"><i />Billing<small>REST</small></span><span className="node node--three"><i />AsyncAPI<small>events</small></span><span className="node node--four"><i />Identity<small>OAuth</small></span><div className="pigeon-network-legend"><span><i /> Healthy</span><span>4 live connections</span></div></div><div className="pigeon-preview-chips">{view.items.map((item) => <span key={item}>{item}</span>)}</div></div>;
...
function LatencyGraph() {
    ...
    const { scrollYProgress } = useScroll({ target: graphRef, offset: ['start 92%', 'end 42%'] });
    ...
    const graphY = useTransform(progress, [0, 1], [28, -28]);
    const circleOpacity = useTransform(progress, [0.25, 0.55], [0.2, 1]);

    return <motion.div ref={graphRef} className="pigeon-latency-graph" aria-hidden="true" style={reduced ? undefined : { y: graphY }}><span>p95 latency</span><svg viewBox="0 0 300 76" preserveAspectRatio="none"><motion.path pathLength="1" strokeDasharray="1" d="M0 57 C32 48 38 61 68 43 S104 53 131 29 S174 45 204 25 S246 36 300 10" style={{ strokeDashoffset: reduced ? 0 : drawOffset }} /><motion.circle cx="204" cy="25" r="4" style={reduced ? undefined : { opacity: circleOpacity }} /></svg><small>84 ms <FiArrowRight /> 62 ms</small></motion.div>;
}

function FeatureVisual({ type }) {
    if (type === 'wide graph') return <LatencyGraph />;
